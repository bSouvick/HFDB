...
cache_dir <- if (length(args) >= 3) args[3] else "cache_spatial_ci_nonlinear_transform"
...
Delta <- 1
lagh  <- c(1, 0)   # change to c(1,1) or c(2,0) as needed
...
base_tag <- paste0("nonlinear_transform_n1_", n1, "_n2_", n2, "_MC_", nMonteCarlo, "_B_", nBoot, "_", lag_tag)
...
##
## Gaussian base field + nonlinear transformation
##
C_fun <- function(phi, nu, d, h, alpha) {
  (phi * (pi^(d/2)) * ((alpha * h)^nu) * BesselK(alpha * h, nu)) /
    (gamma(nu + d/2) * (2^(nu - 1)) * (alpha^(2 * nu)))
}

autocov_fun <- function(h, phi, nu, d, alpha) {
  if (h == 0) {
    C_fun(phi, nu, d, 1e-8, alpha)
  } else {
    C_fun(phi, nu, d, h, alpha)
  }
}
...
gammaX_true <- function(hvec) {
  h <- sqrt(sum(hvec^2))
  autocov_fun(h, phi = phi_base, nu = nu_base, d = d_base, alpha = alpha_base)
}

gammaZ_true <- function(hvec, a_nl) {
  gx <- gammaX_true(hvec)
  gx + 2 * a_nl^2 * gx^2
}

gamma_true <- gammaZ_true(lagh, a_nl)
...
precompute_full <- function() {
  cat("Starting full precomputation for transformed Gaussian field...\n")
  ...
  for (m in seq_len(nMonteCarlo)) {
    ...
    Xvec <- as.numeric(scale(Xvec))
    Zvec <- Xvec + a_nl * (Xvec^2 - 1)
    dataset[, m] <- Zvec
  }
