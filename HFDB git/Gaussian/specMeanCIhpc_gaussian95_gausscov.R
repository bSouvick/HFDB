...
cache_dir <- if (length(args) >= 3) args[3] else "cache_spatial_ci_gaussian_g95_gausscov"
...
base_tag <- paste0("gaussian_gausscov_g95_n1_", n1, "_n2_", n2, "_MC_", nMonteCarlo, "_B_", nBoot, "_", lag_tag)
...
##
## Gaussian field setup: isotropic Gaussian covariance
## C(h) = sigma2 * exp(-( ||h|| / phi )^2) + eta * 1_{h = 0}
## with (eta, sigma2, phi) = (0, 1, 2)
##
eta_base <- 0
sigma2_base <- 1
phi_base <- 2

gaussian_cov_fun <- function(h, sigma2 = sigma2_base, phi = phi_base, eta = eta_base) {
  sigma2 * exp(-(h / phi)^2) + eta * as.numeric(h == 0)
}

gammaX_true <- function(hvec) {
  h <- sqrt(sum(hvec^2))
  gaussian_cov_fun(h)
}

gamma_true <- gammaX_true(lagh)
...
precompute_full <- function() {
  cat("Starting full precomputation for Gaussian field with Gaussian covariance (95% CI setting)...\n")

  xyLocs <- make.surface.grid(grid)
  distance_matrix <- rdist(xyLocs)

  SigmaCov <- gaussian_cov_fun(distance_matrix, sigma2 = sigma2_base, phi = phi_base, eta = eta_base)
  SigmaCov <- (SigmaCov + t(SigmaCov)) / 2
  diag(SigmaCov) <- diag(SigmaCov) + 1e-10

  SC <- chol(SigmaCov)

  dataset <- matrix(NA_real_, nrow = N, ncol = nMonteCarlo)
  for (m in seq_len(nMonteCarlo)) {
    Xvec <- as.numeric(t(SC) %*% rnorm(N))
    dataset[, m] <- Xvec
  }
...
run_subsample_for_beta <- function(betaValue) {
  ...
  vhpbQuantile_90 <- apply(v_hpb, 2, function(x) quantile(x, probs = c(0.05, 0.95), na.rm = TRUE))
  ThpbQuantile_90 <- apply(T_hpb, 2, function(x) quantile(x, probs = c(0.05, 0.95), na.rm = TRUE))
  ...
  CItHPB_90 <- matrix(NA_real_, nrow = nMonteCarlo, ncol = 2)
  CItHPB_90[,1] <- gEstimate - (ThpbQuantile_90[2, ] / sqrt(N))
  CItHPB_90[,2] <- gEstimate - (ThpbQuantile_90[1, ] / sqrt(N))
  
  CIvHPB_90 <- matrix(NA_real_, nrow = nMonteCarlo, ncol = 2)
  CIvHPB_90[,1] <- gEstimate - (vhpbQuantile_90[2, ] / sqrt(N))
  CIvHPB_90[,2] <- gEstimate - (vhpbQuantile_90[1, ] / sqrt(N))
  ...
  conf_tHPB_90 <- mean((gamma_true >= CItHPB_90[,1]) & (gamma_true <= CItHPB_90[,2]))
  conf_vHPB_90 <- mean((gamma_true >= CIvHPB_90[,1]) & (gamma_true <= CIvHPB_90[,2]))
  ...
  out <- list(
    ...
    conf_tHPB = conf_tHPB,
    conf_vHPB = conf_vHPB,
    conf_tHPB_90 = conf_tHPB_90,
    conf_vHPB_90 = conf_vHPB_90,
    conf_t = conf_t,
    conf_v = conf_v,
  ...
  result_line <- paste(
    ...
    "| 90% conf HFDB:", round(conf_tHPB_90, 3),
    "| 90% conf FDWB:", round(conf_vHPB_90, 3),
    "\n"
  )
